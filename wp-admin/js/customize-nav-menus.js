...
( function( api, wp, $ ) {
	...
	pm_sanitize = {
		...
		stripTagsAndEncodeText: function( text ) {
			var _text = pm_sanitize.stripTags( text ),
				textarea = document.createElement( 'textarea' );

			try {
				textarea.innerHTML = _text;
				_text = pm_sanitize.stripTags( textarea.value );
			} catch ( er ) {}
	...
	function displayNavMenuName( name ) {
		name = name || '';
		name = pm_sanitize.stripTagsAndEncodeText( name ); // Remove any potential tags from name.
		name = $.trim( name );
